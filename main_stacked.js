...
const onPlot = async (thisEditor) => {
  ...
  const channelSubset = fileObject.channels.slice(0,5)
  ...
  const relayout = (traces, change={}, plot=true) => {
    ...
    traces.forEach((o,i) => {
      ...
      const changeLayout = case1 || (case21 && case22)
      if (changeLayout){
        const axisName = `yaxis${i+1}`
    ...
    // thisEditor.timeseries.data = traces.map((o,i) => {
    //   const show = 
    //   o.visible = show ? true : 'legendonly'
    //   return o
    // })

    console.log(update)
    if (plot) thisEditor.timeseries.Plotly.relayout(thisEditor.timeseries.div, update)
    return update
  }
  ...
  console.log(thisEditor.timeseries.layout, thisEditor.timeseries.data)

    // Relayout the Channels on the Y Axis
