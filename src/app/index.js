import * as bids from '../standards/bids/src/index.js'
...
const channelsToGet = 4
const editor2 = new components.ObjectEditor({ header: `First EDF File (${channelsToGet} Channels)` })
...
const onPlot = async (thisEditor) => {
    ...
    if (thisEditor === editor2) artifactsDiv.innerHTML = ''
  ...
  const montage = Array.from({length: channelsToGet}, e => true)
  ...
  const channelSubset = fileObject.channels.slice(0,channelsToGet)
...
downloadButton.onClick = async () => {
  if (bidsDataset) {
    ...
    const info = await bidsDataset.zipCheck({}, (ratio) => {
      loader.text = 'Zipping Annotated Dataset'
      loader.progress = ratio
    }, (ratio) => {
    ...
    showValidation(info)
    loader.text = 'Downloading Annotated Dataset'
